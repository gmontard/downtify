  ...
  function fromURL(url, options = {}) {
    ...
    return API.open(url)
      .then((res) => {
        ...
        const data = res.data
        // Playlist responses are objects with { playlist_name, tracks };
        // tracks and albums are still bare arrays / single objects.
        if (
          data &&
          Array.isArray(data.tracks) &&
          data.playlist_name !== undefined
        ) {
          const tracks = data.tracks
          const downloads = tracks.map((song) => {
            console.log('Opened Song:', song)
            return queue(song)
          })
          if (generateM3u && data.playlist_name) {
            Promise.all(downloads).then((results) => {
              const trackPayload = results
